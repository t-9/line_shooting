...
public class SallyRotateTable extends RotateTable{
	
	public SallyRotateTable(float initDegrees, float maxDegrees, float addDegrees, int x, int y) {
		for(float degrees = initDegrees; degrees <= maxDegrees; 
				degrees += addDegrees)
			super.generateRotateBitmap(rotateHashMap, ((AndroidPixmap)(Assets.sally)).getBitmap(), degrees, x, y);
