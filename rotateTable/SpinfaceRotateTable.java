...
public class SpinfaceRotateTable extends RotateTable{
	
	public SpinfaceRotateTable(float initDegrees, float maxDegrees, float addDegrees, int x, int y) {
		for(float degrees = initDegrees; degrees >= maxDegrees; 
				degrees += addDegrees)
			generateRotateBitmap(((AndroidPixmap)(Assets.spin_face01)).getBitmap(), degrees, x, y);
